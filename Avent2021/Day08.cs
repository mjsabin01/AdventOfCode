...
namespace Avent2021
{
    internal class Day08
    {
        ...
        {
            public bool topOn;
            public bool leftTopOn;
            public bool rightTopOn;
            public bool centerOn;
            public bool bottomOn;
            public bool rightBottomOn;
            public bool leftBottomOn;
            ...
            public DigitDisplay(int number, string parts)
            {
                Number = number;
                foreach (var c in parts)
                {
                    switch (c)
                    {
                        case 'a': topOn = true; Mask |= 0b1; NumSegments++;  break;
                        case 'b': leftTopOn = true; Mask |= 0b10; NumSegments++; break; ;
                        case 'c': rightTopOn = true; Mask |= 0b100; NumSegments++; break; ;
                        case 'd': centerOn = true; Mask |= 0b1000; NumSegments++; break; ;
                        case 'e': leftBottomOn = true; Mask |= 0b10000; NumSegments++; break; ;
                        case 'f': rightBottomOn = true; Mask |= 0b100000; NumSegments++; break; ;
                        case 'g': bottomOn = true; Mask |= 0b1000000; NumSegments++; break; ;
                    }
                }
            }
        }

        DigitDisplay[] DisplayVaules = 
        {
            new DigitDisplay(0, "abcefg"),
            new DigitDisplay(1, "cf"),
            new DigitDisplay(2, "acdeg"),
            new DigitDisplay(3, "acdfg"),
            new DigitDisplay(4, "bcdf"),
            new DigitDisplay(5, "abdfg"),
            new DigitDisplay(6, "4abdefg"),
            new DigitDisplay(7, "acf"),
            new DigitDisplay(8, "abcdefg"),
            new DigitDisplay(9, "abcdfg")
        };

        public void Part1(string[] lines)
        {
            var instancesOf1478 = 0;
            foreach (string line in lines)
            {
                ...
                {
                    if (outputNumber.Length == DisplayVaules[1].NumSegments ||
                        outputNumber.Length == DisplayVaules[4].NumSegments ||
                        outputNumber.Length == DisplayVaules[7].NumSegments ||
                        outputNumber.Length == DisplayVaules[8].NumSegments)
                    {
                        instancesOf1478++;
                    }
                }
            ...
            Console.WriteLine($"There are '{instancesOf1478}' instances of 1 | 4 | 7 | 8 in the output values.");
        }
        ...
        public void Part2(string[] lines)
        {
            ...
            {
                var lineParts = line.Split(" | ");
                var inputNumbers = lineParts[0].Split(" ").ToArray();
                ...
                {
                    var mask = 0;
                    foreach (var c in outputNumber.ToCharArray())
                    {
                        mask += segmentDict[c];
                    }

                    val = val * 10 +  DisplayVaules.First(x => x.Mask == mask).Number;
                }

                totalVals += val;
                Console.WriteLine($"- {lineParts[1]}: {val}");
            }

            Console.WriteLine($"Total value is: {totalVals}");
        }

        private Dictionary<char, int> BuildSegmentMaskDictionary(string[] inputNumbers)
        {
            var oneVal = inputNumbers.First(x => x.Length == 2);
            var sevenVal = inputNumbers.First(x => x.Length == 3);
            var fourVal = inputNumbers.First(x => x.Length == 4);
            var eightVal = inputNumbers.First(x => x.Length == 7);


            var sixVal = inputNumbers.First(x => x.Length == 6 && x.ToHashSet().Intersect(oneVal).Count() == 1);
            var nineVal = inputNumbers.First(x => x.Length == 6 && !string.Equals(x, sixVal) && x.ToHashSet().Intersect(fourVal).Count() == 4);
            var zeroVal = inputNumbers.First(x => x.Length == 6 && !string.Equals(x, sixVal) && !string.Equals(x, nineVal));

            var threeVal = inputNumbers.First(x => x.Length == 5 && x.ToHashSet().Intersect(sevenVal).Count() == 3);

            var fiveVal = inputNumbers.First(x => x.Length == 5 && x.ToHashSet().Intersect(sixVal).Count() == 5);

            var twoVal = inputNumbers.First(x => x.Length == 5 && x.ToHashSet().Intersect(sixVal).Count() == 4);

            var charMaskDict = new Dictionary<char, int>();

            // top char is the one in 7 but not 1
            var topChar = sevenVal.Except(oneVal).First();
            charMaskDict.Add(topChar, 1);

            // top left is extra one that 9 has but 3 doesn't
            var topLeftChar = nineVal.Except(threeVal).First();
            charMaskDict.Add(topLeftChar, 2);

            // top right is extra one that 9 has that 6 doesn't
            var topRightChar = nineVal.Except(sixVal).First();
            charMaskDict.Add(topRightChar, 4);

            // center is extra one that 8 has that 0 doesn't
            var centerChar = eightVal.Except(zeroVal).First();
            charMaskDict.Add(centerChar, 8);

            // bottom left is extra one that 6 has that 5 doesn't
            var bottomLeftChar = sixVal.Except(fiveVal).First();
            charMaskDict.Add(bottomLeftChar, 16);

            // bottom right is the one in 1 that isn't top right
            var bottomRightChar = oneVal.Except(new[] { topRightChar }).First();
            charMaskDict.Add(bottomRightChar, 32);

            // bottom char is remaining char in 8
            var bottomChar = eightVal.Except(new[] { topChar, topRightChar, topLeftChar, centerChar, bottomLeftChar, bottomRightChar }).First();
            charMaskDict.Add(bottomChar, 64);
