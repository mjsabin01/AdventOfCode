...
namespace Avent2021
{
    internal class Day19
    {
        ...
        List<Scanner> ParseInput(string[] lines)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                {
                    scanners.Add(new Scanner(lines, lastStartIndex, i - 1));
                    lastStartIndex = i + 1;
                }
            }

            scanners.Add(new Scanner(lines, lastStartIndex, lines.Length - 1));
            return scanners;
        }
        ...
        public class Scanner
        {
            ...
            public List<ScannerBeacon> Beacons { get; set; } = new List<ScannerBeacon>();

            public Dictionary<ScannerBeacon, List<long>> BeaconDistances = new Dictionary<ScannerBeacon, List<long>>();

            public Scanner (string[] lines, int startIndex, int endIndex)
            {
                Id = lines[startIndex];

                for (int i = startIndex + 1; i <= endIndex; i++)
                {
                    Beacons.Add(new ScannerBeacon(lines[i], this));
                }

                foreach (var beacon in Beacons)
                {
                    var beaconDistances = new List<long>();
                    BeaconDistances[beacon] = beaconDistances;
                    foreach (var otherBeacon in Beacons)
                    {
                        var distance = beacon.GetDistance(otherBeacon);
                        beaconDistances.Add(distance);
                    }
            ...
            public void CheckForAssociatedScanners(Scanner other)
            {
                var myIntersectingBecons = new HashSet<ScannerBeacon>();
                foreach (var otherBeacon in other.Beacons)
                {
                    foreach (var beacon in this.Beacons)
                    {
                        if (myIntersectingBecons.Contains(beacon))
                        {
                            continue;
                        }

                        var myBeconDistances = BeaconDistances[beacon];
                        var otherBeconDistances = other.BeaconDistances[otherBeacon];

                        var intersectingDistnaces = myBeconDistances.Intersect(otherBeconDistances);
                        var intersectingCount = intersectingDistnaces.Count();
                        if (intersectingCount >= 12)
                        {
                            beacon.ConnectedBeacons.Add(otherBeacon);
                            otherBeacon.ConnectedBeacons.Add(beacon);
                            myIntersectingBecons.Add(beacon);                            
                            break;
                        }
                    }
                }
        ...
        public class ScannerBeacon
        {
            ...
            public List<Coordinate3d> Permutations { get; set; } = new List<Coordinate3d>();

            public HashSet<ScannerBeacon> ConnectedBeacons { get; set; } = new HashSet<ScannerBeacon>();
        ...
        {
            public HashSet<ScannerBeacon> Becons { get; set; } = new HashSet<ScannerBeacon>();

            public HashSet<Scanner> Scaners { get; set; } = new HashSet<Scanner>();
