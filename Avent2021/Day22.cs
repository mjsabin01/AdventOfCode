...
namespace Avent2021
{
    internal class Day22
    {
        ...
        public void Part1(string[] lines)
        {
            ...
            for (var x = 0; x <= 100; x++)
            {
                for (var y = 0; y <= 100; y++)
                {
                    for (var z = 0; z <= 100; z++)
                    {
                        onCount += cube[x, y, z] ? 1 : 0;
                    }
                }
            }

            Console.WriteLine("There are {0} cubes on.", onCount);
        }
        ...
        public void Part2(string[] lines)
        {
            ...
            foreach (var instruction in instructions)
            {
                foreach (var addInstruction in addInstructions)
                {
                    addInstruction.AddInstructionIfItIntersects(instruction);
                }
            ...
            long numberOn = 0;
            foreach (var instrcution in addInstructions)
            {
                numberOn += instrcution.GetNumberPointsOn();
            }

            Console.WriteLine("Total number of on points: {0}", numberOn);
        }
        ...
        public class Area
        {
            ...
            public long Size => (MaxX - MinX) * (MaxY - MinY) * (MaxZ - MinZ);

            public Area(long minX, long minY, long minZ, long maxX, long maxY, long maxZ)
            {
                MinX = minX;
                MaxX = maxX;
                MinY = minY;
            ...
            public Area GetIntersection(Area other)
            {
                if (!DoesOtherAreaIntersect(other))
                {
                    return new Area(0, 0, 0, 0, 0, 0);
                }

                var minX = Math.Max(other.MinX, this.MinX);
                var maxX = Math.Min(other.MaxX, this.MaxX);
                var minY = Math.Max(other.MinY, this.MinY);
                var maxY = Math.Min(other.MaxY, this.MaxY);
                var minZ = Math.Max(other.MinZ, this.MinZ);
                var maxZ = Math.Min(other.MaxZ, this.MaxZ);

                return new Area(minX, minY, minZ, maxX, maxY, maxZ);
            }
            public override string ToString()
            {
                return $"Size: {Size} (X:{MinX} - {MaxX} Y:{MinY} - {MaxY} Z: {MinZ} - {MaxZ})";
            }
        ...
        public class Instruction
        {
            ...
            public Instruction(string input, int order)
            {
                ...
                IsOn = string.Equals("on", input.Substring(0, spaceIndex));
                var coordParts = input.Substring(spaceIndex + 1).Split(',');
            ...
            public void AddInstructionIfItIntersects(Instruction i)
            {
            ...
            public override string ToString()
            {
                var onString = IsOn ? "on" : "off";
                return $"{onString} {Area}";
            }
        }
