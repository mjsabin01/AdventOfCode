...
namespace Avent2020
{
    class Day02
    {
        private static string TestInput = @"1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc";
        ...
        public void Run1()
        {
            ...
            foreach (var line in lines)
            {
                ...
                var letterCount = str.Split(letter).Length - 1;
                var isMatch = letterCount >= min && letterCount <= max;
                Console.WriteLine($"{letterCount} instances of {letter} ({min} - {max} expected) in input: {str}.");
                if (isMatch)
                {
                    matchCount++;
                }
        ...
        public void Run2()
        {
            ...
            foreach (var line in lines)
            {
                ...
                var match = Regex.Match(line);
                var firstPos = int.Parse(match.Groups[1].Value) - 1;
                var secondPos = int.Parse(match.Groups[2].Value) - 1;
                var letter = match.Groups[3].Value[0];
                var str = match.Groups[4].Value;

                if (str.Length < secondPos)
                {
                    continue;
                }


                var isValid = (str[firstPos] == letter ^ str[secondPos] == letter);
                Console.WriteLine($"Input string valid: {isValid} for one instance of {letter} at index {firstPos} or {secondPos} in input: {str}.");
                if (isValid)
                {
                    matchCount++;
                }
        ...
        private static string Input = @"3-4 t: dttt
5-7 l: llmlqmblllh
