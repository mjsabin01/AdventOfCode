...
namespace Avent2020
{
    class Day07
    {
        class Bag
        {
            ...
            public ISet<Bag> Parents { get; set; } = new HashSet<Bag>();
        ...
        public void Problem1()
        {
            ...
            ParseInput(Input, bagDict);
            var expectedBag = bagDict["shiny gold"];
            var parents = new HashSet<Bag>();
            foreach (var bag in bagDict.Values)
            {
                if (bag == expectedBag)
                    continue;
                CheckChildren(bag, expectedBag, parents);
            }

            Console.WriteLine($"Unique outer bags: {string.Join(",", parents.Select(x => x.Name))} {parents.Count}.");
        }
        ...
        void Problem2()
        {
            ...
            ParseInput(Input, bagDict);
            var expectedBag = bagDict["shiny gold"];
            var count = GetChildrenCount(expectedBag) - 1;
            Console.WriteLine($"Total inner bags is: {count}.");
        }

        bool CheckChildren(Bag bag, Bag expectedBag, HashSet<Bag> parents)
        {
            ...
            {
                if (child == expectedBag)
                {
                    parents.Add(bag);
                    return true;
                }
                if (CheckChildren(child, expectedBag, parents))
                {
                    parents.Add(bag);
                    return true;
                }
        ...
        long GetChildrenCount(Bag bag)
        {
            ...
            {
                sum += child.Value * GetChildrenCount(child.Key);
            }

            return sum;
        }

        void ParseInput(string input, Dictionary<string, Bag> bagDict)
        {
            var lines = input.Split("\r\n");
            var regChild = new Regex(@"^(\d+) ([a-zA-Z ]+) bags?$");
            foreach (var line in lines)
            {
                var parts = line.Trim('.').Split(" contain ");
                var parent = parts[0].Replace(" bags", "").Replace(" bag", "");
                var children = parts[1].Split(", ");
                Console.WriteLine($"Parent bag: {parent}.");

                if (!bagDict.ContainsKey(parent))
                {
                    bagDict[parent] = new Bag() { Name = parent };
                }
                var parentBag = bagDict[parent];
                ...
                {
                    var match = regChild.Match(child);
                    if (!match.Success)
                    {
                        continue;
                    }
                    ...
                    Console.WriteLine($"--Child bag: {bagName}.");
                    if (!bagDict.ContainsKey(bagName))
                    {
                        bagDict[bagName] = new Bag() { Name = bagName };
                    }
                    var childBag = bagDict[bagName];
...
dotted black bags contain no other bags.";
        ...
        string TestInput2 = @"shiny gold bags contain 2 dark red bags.
...
dark violet bags contain no other bags.";
        ...
        string Input = @"muted lime bags contain 1 wavy lime bag, 1 vibrant green bag, 3 light yellow bags.
