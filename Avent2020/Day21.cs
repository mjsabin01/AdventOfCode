...
namespace Avent2020
{
    class Day21
    {
        public class Allergen
        {
            ...
            public HashSet<Recipe> AssociatedRecipies { get; } = new HashSet<Recipe>();

            public HashSet<Ingredient> PossibleIngredients { get; } = new HashSet<Ingredient>();
            ...
            {
                return $"{Name}: Associated Recipies: {AssociatedRecipies.Count} Number of Possible Ingredient: {PossibleIngredients.Count} Allergen: {Ingredient?.Name}";
            }
        }
        public class Ingredient
        {
            ...
            public HashSet<Allergen> PossibleAlergens { get; } = new HashSet<Allergen>();
            ...
            public override string ToString()
            {
                return $"{Name}: Number of Possible Alergens: {PossibleAlergens.Count} Allergen: {Allergen?.Name} Number of apperances: {NumberOfAppearances}.";
            }
        }

        public class Recipe
        {
            public HashSet<Allergen> Allergens { get; set; } = new HashSet<Allergen>();
        ...
        HashSet<Recipe> AllRecipess { get; set; } = new HashSet<Recipe>();

        Regex RegLine = new Regex(@"(\w+ )+\(contains( (\w+),?)+\)");

        public void Run()
        {
            ParseInput1();
            var orderedAlergies = AllAllergens.Values.ToList();
            orderedAlergies.Sort((x, y) => x.AssociatedRecipies.Count == y.AssociatedRecipies.Count ?
                x.PossibleIngredients.Count.CompareTo(y.PossibleIngredients.Count) :
                y.AssociatedRecipies.Count.CompareTo(x.AssociatedRecipies.Count));

            while (orderedAlergies.Any(x => x.Ingredient == null))
            {
                foreach (var alergie in orderedAlergies)
                {
                    if (alergie.Ingredient != null)
                        continue;

                    IEnumerable<Ingredient> ingredients = alergie.PossibleIngredients.Where(x => x.Allergen == null);
                    foreach (var recipie in alergie.AssociatedRecipies)
                    {
                        var possibleIngredients = recipie.Ingredients.Where(x => x.Allergen == null);
                        ingredients = ingredients.Intersect(recipie.Ingredients);
                    }

                    var numPossibleIngredients = ingredients.Count();
                    if (numPossibleIngredients != 1)
                    {
                        continue;
                        throw new Exception();
                    }

                    AssignIngredient(ingredients.First(), alergie);
                }
            }

            var noAlergenIngredients = AllIngredients.Values.Where(x => x.Allergen == null).ToList();
            Console.WriteLine($"There are: {noAlergenIngredients.Count} ingredients with no alergens: {string.Join(", ", noAlergenIngredients.Select(x => x.Name))}");
            Console.WriteLine($"Total number of apperances of ingredients with no alergens: {noAlergenIngredients.Sum(x => x.NumberOfAppearances)}");

            orderedAlergies.Sort((x, y) => string.Compare(x.Name, y.Name));
            Console.WriteLine($"Ingredients are: {string.Join(",", orderedAlergies.Select(x => x.Ingredient.Name.Trim()))}");
        }

        public void AssignIngredient(Ingredient ingredient, Allergen alergen)
        {
            Console.WriteLine($"Ingredient: {ingredient.Name} alergen is: {alergen.Name}.");

            ingredient.Allergen = alergen;
            alergen.Ingredient = ingredient;
            ingredient.PossibleAlergens.Clear();
            alergen.PossibleIngredients.Remove(ingredient);

            foreach (var otherIngredient in alergen.PossibleIngredients)
            {
                otherIngredient.PossibleAlergens.Remove(alergen);
                alergen.PossibleIngredients.Remove(otherIngredient);
            }
        }
        ...
        public void ParseInput1()
        {
            ...
            foreach (var line in lines)
            {
                ...
                for(var i = 0; i < match.Groups[1].Captures.Count; i++)
                {
                    ...
                    {
                        AllIngredients[name] = new Ingredient { Name = name };
                    }
                    AllIngredients[name].NumberOfAppearances++;
                    ingredients.Add(AllIngredients[name]);
                    recipe.Ingredients.Add(AllIngredients[name]);
                }

                for (var i = 0; i < match.Groups[3].Captures.Count; i++)
                {
                    var name = match.Groups[3].Captures[i].Value;
                    if (!AllAllergens.ContainsKey(name))
                    {
                        AllAllergens[name] = new Allergen { Name = name };
                    }
                    var alergen = AllAllergens[name];

                    foreach (var ingredient in ingredients)
                    {
                        ingredient.PossibleAlergens.Add(alergen);
                        alergen.PossibleIngredients.Add(ingredient);
                    }

                    recipe.Allergens.Add(alergen);
                    alergen.AssociatedRecipies.Add(recipe);
                }
            }
        }

        string TestInput = @"mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
trh fvjkl sbzzf mxmxvkd (contains dairy)
sqjhc fvjkl (contains soy)
sqjhc mxmxvkd sbzzf (contains fish)";

        string Input = @"hhbf zkzls sbkb krd msmnzlm pbfnr szpbd dgbz mjrrfzz dzrjg mttl slgdh nmgqt hqkxv nchrg dvknd fpcfdg jtrgt nrrll cbmbhft rjnmg dzrzc vgfkcnr mtdsv tbthrm hdsm dml xchzh kfgd gzlqd khhf sjhds zk jxbqcp mzvkx nbgklf qlqrm rkjjsl fvszcb mzj kncpfb pdkcrm gvnm vvkrmf fttbhdr qxgtsm qjxxpr kmr xbfht kbrr bxvkx clvr rmblkb dgdq shqqg rldlxf qqfnqb gzbhn vzj gtqqlgl hnctrp (contains eggs, fish)
